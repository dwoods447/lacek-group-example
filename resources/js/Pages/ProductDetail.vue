<template>
  ...
  <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
    ...
    <div class="lg:max-w-lg lg:self-end">
      ...
      <div class="mt-4">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{{ props.product[0].name }}</h1>
      </div>
      ...
      <section aria-labelledby="information-heading" class="mt-4">
        ...
        <div class="flex items-center">
          <p class="text-lg text-gray-900 sm:text-xl">${{ props.product[0].price }}</p>
    ...
    <div class="mt-10 lg:col-start-2 lg:row-span-2 lg:mt-0 lg:self-center">
      <div class="aspect-h-1 aspect-w-1 overflow-hidden rounded-lg">
        <img :src="props.product[0].image_url" alt="Model wearing light green backpack with black canvas straps and front zipper pouch." class="h-full w-full object-cover object-center">
      </div>
    </div>
...
async function addItemToCart(e) {
  e.preventDefault();
  const resp = await fetch(`/add/product/${props.product[0].id}/cart`);
  const response = await resp.json();
  router.visit(`/view/cart`, {
    method: 'get',
    data: {
